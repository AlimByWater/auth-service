...
func main() {
	...
	router.HandleFunc("/token/{guid}", createToken).Methods("GET")
	router.HandleFunc("/refresh", Refresh).Methods("POST")
	router.HandleFunc("/token", removeToken).Methods("DELETE")
	router.HandleFunc("/token/{guid}", removeAllTokens).Methods("DELETE")
